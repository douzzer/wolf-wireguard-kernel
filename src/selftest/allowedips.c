...
static __init bool randomized_test(void)
{
	...
	for (i = 0; i < NUM_RAND_ROUTES; ++i) {
		wc_get_random_bytes_nofail(ip, 4);
		cidr = wc_get_random_u32_below(32) + 1;
		peer = peers[wc_get_random_u32_below(NUM_PEERS)];
		...
		for (j = 0; j < NUM_MUTATED_ROUTES; ++j) {
			memcpy(mutated, ip, 4);
			wc_get_random_bytes_nofail(mutate_mask, 4);
			mutate_amount = wc_get_random_u32_below(32);
	...
	for (i = 0; i < NUM_RAND_ROUTES; ++i) {
		wc_get_random_bytes_nofail(ip, 16);
		cidr = wc_get_random_u32_below(128) + 1;
		peer = peers[wc_get_random_u32_below(NUM_PEERS)];
		...
		for (j = 0; j < NUM_MUTATED_ROUTES; ++j) {
			...
			wc_get_random_bytes_nofail(mutate_mask, 16);
			mutate_amount = wc_get_random_u32_below(128);
			for (k = 0; k < mutate_amount / 8; ++k)
				mutate_mask[k] = 0xff;
			mutate_mask[k] = 0xff
					 << ((8 - (mutate_amount % 8)) % 8);
