...
insert: debug
	-modinfo -F depends wolfguard.ko | tr ',' '\n' | sudo xargs -n 1 modprobe
	-sudo rmmod wolfguard
	-sudo insmod wolfguard.ko
...
remote-test:
	...
	rsync --rsh="ssh $(SSH_OPTS1)" $(RSYNC_OPTS) . $(REMOTE_HOST1):wolfguard-build/
	ssh $(SSH_OPTS1) $(REMOTE_HOST1) 'make -C wolfguard-build test -j$$(nproc)'
	ssh $(SSH_OPTS1) -O exit $(REMOTE_HOST1)

remote-run-1:
	ssh $(SSH_OPTS1) -Nf $(REMOTE_HOST1)
	rsync --rsh="ssh $(SSH_OPTS1)" $(RSYNC_OPTS) . $(REMOTE_HOST1):wolfguard-build/
	ssh $(SSH_OPTS1) $(REMOTE_HOST1) 'ip l d wg0; rmmod wolfguard; cd wolfguard-build && make -j$$(nproc) $(MAYBE_DEBUG) && make install'
	ssh $(SSH_OPTS1) $(REMOTE_HOST1) 'ip l a wg0 type wolfguard'
...
remote-run-2:
	...
	rsync --rsh="ssh $(SSH_OPTS2)" $(RSYNC_OPTS) . $(REMOTE_HOST2):wolfguard-build/
	ssh $(SSH_OPTS2) $(REMOTE_HOST2) 'ip l d wg0; rmmod wolfguard; cd wolfguard-build && make -j$$(nproc) $(MAYBE_DEBUG) && make install'
	ssh $(SSH_OPTS2) $(REMOTE_HOST2) 'ip l a wg0 type wolfguard'
...
remote-run-3:
	...
	rsync --rsh="ssh $(SSH_OPTS3)" $(RSYNC_OPTS) . $(REMOTE_HOST3):wolfguard-build/
	ssh $(SSH_OPTS3) $(REMOTE_HOST3) 'ip l d wg0; rmmod wolfguard; cd wolfguard-build && make -j$$(nproc) $(MAYBE_DEBUG) && make install'
	ssh $(SSH_OPTS3) $(REMOTE_HOST3) 'ip l a wg0 type wolfguard'
