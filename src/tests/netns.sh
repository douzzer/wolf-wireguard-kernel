...
# wolfguard peers in $ns1 and $ns2. Note that $ns0 is the endpoint for the wg0
...
ip0 link add dev wg0 type wolfguard
...
ip1 link add wg1 type wolfguard
ip2 link add wg1 type wolfguard
...
ip1 link add dev wg0 type wolfguard
ip2 link add dev wg0 type wolfguard
configure_peers
...
((i == 40))
ip0 link del wg0
ip0 link add wg0 type wolfguard
config=( )
for i in {1..29}; do
	config+=( "[Peer]" "PublicKey=$(wg genkey)" )
done
...
ip1 link add wg1 type wolfguard
ip2 link add wg2 type wolfguard
